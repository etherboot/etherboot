...
int tftp_receive_file(struct client_data *data)
{
     ...
     char mc_addr[24];          /* multicast address */
     char string[MAXLEN];
     ...
     while (1)
     {
          ...
          case S_OACK_RECEIVED:
               ...
               if ((result = opt_get_multicast(data->tftp_options_reply,
                                               mc_addr, &mc_port,
                                               &data->master_client)) > 0)
               {
                    if (trace)
                         fprintf(stderr, "multicast: %s,%d %d, ", mc_addr,
                                 mc_port, data->master_client);
                    if (!inet_aton(mc_addr, &data->sa_mcast.sin_addr))
                    {
                         fprintf(stderr, "tftp: bad multicast address");
                         exit(1);
                    }
                    data->sa_mcast.sin_port = htons(mc_port);
                    multicast = 1;
               }
               if (trace)
                    fprintf(stderr, "\b\b>\n");
