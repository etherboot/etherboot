...
void stats_print(void)
{
     ...
     logger(LOG_NOTICE, "  Load measurements:");
     logger(LOG_NOTICE, "   User: %8.3fs  Sys:%8.3fs",
            (double)(s_stats.tms.tms_utime) / CLK_TCK,
            (double)(s_stats.tms.tms_stime) / CLK_TCK);
     logger(LOG_NOTICE, "   Total:%8.3fs  CPU:%8.3f%%", 
            (double)(tmp.tv_sec + tmp.tv_usec * 1e-6),
            (double)(s_stats.tms.tms_utime + s_stats.tms.tms_stime) /
            (double)(tmp.tv_sec + tmp.tv_usec * 1e-6));
     logger(LOG_NOTICE, "  Time between connections:");
     if (s_stats.min_time.tv_sec == LONG_MAX)
          logger(LOG_NOTICE, "   Min: -----   Max: -----");
     else
          logger(LOG_NOTICE, "   Min: %.3fs Max: %.3fs",
                 (double)(s_stats.min_time.tv_sec + s_stats.min_time.tv_usec * 1e-6),
                 (double)(s_stats.max_time.tv_sec + s_stats.max_time.tv_usec * 1e-6));
     logger(LOG_NOTICE, "  Thread stats:");
     logger(LOG_NOTICE, "   simultaneous threads:     %d", s_stats.max_simul_threads);
     logger(LOG_NOTICE, "   number of servers:        %d", s_stats.number_of_server);
     logger(LOG_NOTICE, "   number of aborts:         %d", s_stats.number_of_abort);
     logger(LOG_NOTICE, "   number of errors:         %d", s_stats.number_of_err);
     logger(LOG_NOTICE, "   number of files sent:     %d", s_stats.num_file_send);
     logger(LOG_NOTICE, "   number of files received: %d", s_stats.num_file_recv);
}
