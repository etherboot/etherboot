          ...
          {
               {
                    openlog(log_ident, LOG_PID, LOG_DAEMON);
                    log_syslog_is_open = 1;
                    logger(LOG_CRIT, "Unable to open %s for logging, reverting to"
                           "syslog", log_filename);
               } 
          }
     }
...
/*
 * Same as syslog but allow to format a string, like printf, when logging to file.
 * This fonction will either call syslog or fprintf depending of the previous
 * call to open_logger().
 */
