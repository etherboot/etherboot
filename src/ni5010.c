	...
	const char *p)	/* Packet */
{
	...
	outsb(IE_XBUF, d, ETHER_ADDR_SIZE);	/* Put dst in buffer */
	outsb(IE_XBUF, nic->node_addr, ETHER_ADDR_SIZE);/* Put src in buffer */
	outb(t >> 8, IE_XBUF);
	outb(t, IE_XBUF);
	outsb(IE_XBUF, p, s);	/* Put data in buffer */
	while (s++ < ETH_MIN_PACKET - ETHER_HDR_SIZE)	/* Pad to min size */
		outb(0, IE_XBUF);
