	...
	const char *p)			/* Packet */
{
	...
	*/
	...
	memcpy_toio(mem, d, ETHER_ADDR_SIZE);
	memcpy_toio(mem + ETHER_ADDR_SIZE, nic->node_addr, ETHER_ADDR_SIZE);
	mem[ETHER_ADDR_SIZE * 2] = t >> 8;
	mem[ETHER_ADDR_SIZE * 2 + 1] = t;
	memcpy_toio(mem + ETHER_HDR_SIZE, p, s);
	s += ETHER_HDR_SIZE;
	len = (s < ETH_MIN_PACKET ? ETH_MIN_PACKET : s);
	/* clean out flags */
