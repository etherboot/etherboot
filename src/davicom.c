/*  
    ...
    You can get Tulip driver source file from this web.
                           ...
                           unsigned int s, const char *p)
{
  ...
  memcpy(&txb[0], d, ETHER_ADDR_SIZE);	/* DA 6byte */
  memcpy(&txb[ETHER_ADDR_SIZE], nic->node_addr, ETHER_ADDR_SIZE); /* SA 6byte*/
  txb[ETHER_ADDR_SIZE*2] = (t >> 8) & 0xFF; /* Frame type: 2byte */
  txb[ETHER_ADDR_SIZE*2+1] = t & 0xFF;
  memcpy(&txb[ETHER_HDR_SIZE], p, s); /* Frame data */

  /* setup the transmit descriptor */
  txd[TxPtr].buf1sz   = ETHER_HDR_SIZE+s;
