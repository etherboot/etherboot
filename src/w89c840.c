 ...
 *
 * Frankly speaking, about 90% of the code was obtained using cut'n'pase
    ...
    const char *p)            /* Packet */
{
    ...
    unsigned entry;
    int transmit_loop_counter = TX_LOOP_COUNT;
    int transmit_status;
    ...
    memcpy ((void *) tx_packet, d, ETHER_ADDR_SIZE);    /* dst */
    memcpy ((void *) tx_packet + ETHER_ADDR_SIZE, nic->node_addr, ETHER_ADDR_SIZE);/* src */

    *((char *) tx_packet + 12) = t >> 8;    /* type */
    *((char *) tx_packet + 13) = t;

    memcpy ((void *) tx_packet + ETHER_HDR_SIZE, p, s);
    s += ETHER_HDR_SIZE;

    while (s < ETH_MIN_PACKET)
    *((char *) tx_packet + ETHER_HDR_SIZE + (s++)) = 0;
...

    {
        ...
        while (1) {
            ...
            if (intr_stat & (NormalIntr | IntrTxDone)) {
                ...
                while ( (transmit_status & DescOwn) && --transmit_loop_counter) {
...
struct nic *w89c840_probe(struct nic *nic, unsigned short *probe_addrs, struct pci_device *p)
{
    ...
    pcibios_read_config_word(p->pci_bus_no, p->devfn, PCI_COMMAND, &pci_command);
    new_command = pci_command | PCI_COMMAND_MASTER|PCI_COMMAND_IO;

    if (pci_command != new_command) {
        printf("\nThe PCI BIOS has not enabled this device!\n"
               "Updating PCI command %x->%x. pci_bus %x pci_device_fn %x\n",
               pci_command, new_command, p->pci_bus_no, p->devfn);
        pcibios_write_config_word(p->pci_bus_no, p->devfn, PCI_COMMAND, new_command);
    }

    pcibios_read_config_byte(p->pci_bus_no, p->devfn, PCI_LATENCY_TIMER, &pci_latency);
    if (pci_latency < 32) {
        printf("\nPCI latency timer (CFLT) is unreasonably low at %d. "
               "Setting to 32 clocks.\n", pci_latency);
        pcibios_write_config_byte(p->pci_bus_no, p->devfn, PCI_LATENCY_TIMER, 32);
    }
