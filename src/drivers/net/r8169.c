...
static int
rtl8169_init_board (struct pci_device *pdev)
{
  int i;			//, pm_cap;
...
r8169_poll (struct nic *nic)
{
  ...
  if ((tpc->RxDescArray[cur_rx].status & OWNbit) == 0)
      ...
      if (!(tpc->RxDescArray[cur_rx].status & RxRES))
	{
	  nic->packetlen = (int) (tpc->RxDescArray[cur_rx].
				  status & 0x00001FFF) - 4;
	  memcpy (nic->packet, tpc->RxBufferRing[cur_rx], nic->packetlen);
	  if (cur_rx == NUM_RX_DESC - 1)
	    tpc->RxDescArray[cur_rx].status = (OWNbit | EORbit) + RX_BUF_SIZE;
	  else
	    tpc->RxDescArray[cur_rx].status = OWNbit + RX_BUF_SIZE;
  ...
  tpc->cur_rx = cur_rx;
  //FIXEME: There is no reason to do this as cur_rx did not change
...
***************************************************************************/
static void
r8169_transmit (struct nic *nic, const char *d,	/* Destination */
		unsigned int t,	/* Type */
		unsigned int s,	/* size */
		const char *p)
{				/* Packet */
  ...
  if (entry != (NUM_TX_DESC - 1))
    tpc->TxDescArray[entry].status =
      (OWNbit | FSbit | LSbit) | ((s > ETH_ZLEN) ? s : ETH_ZLEN);
  else
    tpc->TxDescArray[entry].status =
      (OWNbit | EORbit | FSbit | LSbit) | ((s > ETH_ZLEN) ? s : ETH_ZLEN);
...
static void
rtl8169_set_rx_mode (struct nic *nic __unused)
{
...
static void
rtl8169_hw_start (struct nic *nic)
{
  u32 i;
...
static void
rtl8169_init_ring (struct nic *nic __unused)
{
  int i;
...
#define valid_link 0
static int
r8169_probe (struct dev *dev, struct pci_device *pci)
{
  ...
  printf ("%s: %! at ioaddr %hX, ", pci->name, nic->node_addr, ioaddr);
  ...
  if (!(RTL_R8 (PHYstatus) & TBI_Enable))
	  ...
	  if (mdio_read (PHY_STAT_REG) & PHY_Auto_Neco_Comp)
	    {
	      ...
	      if (option & _1000bpsF)
		{
		  printf ("1000Mbps Full-duplex operation.\n");
		}
	      else
		{
		  printf
	...
	("%s: 1000Mbps Full-duplex operation, TBI Link %s!\n",
	 pci->name, (RTL_R32 (TBICSR) & TBILinkOK) ? "OK" : "Failed");
