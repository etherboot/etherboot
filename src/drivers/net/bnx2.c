...
static void
bnx2_set_rx_mode(struct nic *nic)
{
	struct bnx2 *bp = &bnx2;
...
static void
bnx2_disable(struct dev *dev __unused)
{
	struct bnx2* bp = &bnx2;
		...
		unsigned int type, unsigned int size, const char *packet)
{
	...
	while((hw_cons != prod) && (hw_cons != (PREV_TX_BD(prod)))) {
		...
		if (++i > 500) { /* timeout 5s for transmit */
			...
			bnx2_disable((struct dev *)bp->nic);
...
static int
bnx2_probe(struct dev *dev, struct pci_device *pdev)
{
...
static struct pci_id bnx2_nics[] = {
