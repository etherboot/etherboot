...
static unsigned short swab(unsigned short s)
{
	return (((s & 0xFF) << 8) | (s >> 8));
}
...
static int eepro_probe1(struct nic *nic)
{
	...
	station_addr.saddr[0] = swab(station_addr.saddr[0]);
	station_addr.saddr[1] = swab(station_addr.saddr[1]);
	station_addr.saddr[2] = swab(station_addr.saddr[2]);
